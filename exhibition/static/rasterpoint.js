class RasterPoint {
  ...
  update(tempScreen, factor) {
    ...
    for (var y=-1; y<=1; y++) {
      for (var x=-1; x<=1; x++) {
        var pointColor = tempScreen.get(int((this.position.x+(gridSize*x))/factor), int((this.position.y+(gridSize*y))/factor));
  ...
  element1() {
    ...
    bezierVertex(this.elementSize, this.elementSize*magicNr, this.elementSize*magicNr, this.elementSize, 0, this.elementSize);
    endShape(CLOSE);
  }
